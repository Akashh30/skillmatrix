...
export default function DisplayCourseSubskillMapping() {
  ...
  const filteredData = data.filter((item) => {
    ...
    // 2. Course Category
    if (courseCategoryFilter && item.course_category !== courseCategoryFilter) {
      return false;
    }
    // 3. Skill Name
  ...
  const courseCategoryOptions = [
    ...new Set(filteredData.map((item) => item.course_category)),
  ];
  const skillNameOptions = [...new Set(filteredData.map((item) => item.skill_name))];
  const skillTypeOptions = [...new Set(filteredData.map((item) => item.type_name))];
